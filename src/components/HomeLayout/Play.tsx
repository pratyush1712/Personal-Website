function _interopDefault(ex: { [x: string]: any }) {
...
function _extends<T>(target: T, ...sources: Partial<T>[]): T {
...
function BubbleElement(props: { children: string | any[]; options: any; className: any; style: any; loadMore: any }) {
	...
	options.numCols = Math.min(options?.numCols, childrenState.length);
	var minProportion = options?.minSize / options?.size;
	var verticalPadding =
		"calc(50% - " + (options?.yRadius + options?.size / 2 - (options?.cornerRadius * (1.414 - 1)) / 1.414) + "px)";
	var horizontalPadding =
		"calc(50% - " + (options?.xRadius + options?.size / 2 - (options?.cornerRadius * (1.414 - 1)) / 1.414) + "px)";
	var scrollable = React.useRef(null);
	var rows: any[][] = [];
	var colsRemaining = 0;
	var evenRow = true;

	for (var i = 0; i < childrenState.length; i++) {
		if (colsRemaining == 0) {
		...
		rows[rows.length - 1].push(childrenState[i]);
		colsRemaining--;
	}
	...
	var handleScroll = function handleScroll(e: any) {
	...
	var getBubbleSize = function getBubbleSize(row: number, col: number) {
		var yOffset =
		...
		if (distanceFromEdge > 0 && distanceFromEdge <= options?.fringeWidth) {
			...
			var extra =
				(Math.max(0, distanceFromEdge - options?.fringeWidth - options?.size / 2) * options?.gravitation) / 10;
							...
							row.map(function (comp, j) {
								...
								return /*#__PURE__*/ React__default.createElement(
									...
									options?.provideProps
										? React__default.cloneElement(comp, {
