...
function stripUrls(input: string): string {
  return input
    .replace(/https?:\/\/\S+/gi, " ")
    .replace(
      /\b[\w-]+\.(?:com|org|net|io|dev|app|ai|co|life|edu|gov|me|xyz)\b/gi,
      " ",
    );
}
...
function buildIndex(chunks: PortfolioChunk[]): PortfolioIndex {
  const indexed: IndexedChunk[] = chunks.map((chunk) => {
    ...
    const normalizedAliases = Array.from(
      new Set(
        chunk.aliases.map(normalize).filter((alias) => alias.length >= 3),
      ),
    );
...
function scoreChunk(
  ...
  const hasSignal =
    matchedAliases.length > 0 || titleHits > 0 || hasStrongKeyword;
...
function compareScored(a: ScoredChunk, b: ScoredChunk): number {
  ...
  return a.item.chunk.id < b.item.chunk.id
    ? -1
    : a.item.chunk.id > b.item.chunk.id
      ? 1
      : 0;
}
...
export function retrievePortfolioContext(
  ...
  const maxChunks = positiveInt(
    String(options.maxChunks ?? ""),
    DEFAULT_MAX_CHUNKS,
  );
  ...
  const scored = index.chunks
    .map((item) =>
      scoreChunk(item, queryTokens, paddedQuery, index.idf, signaledKinds),
    )
  ...
  if (signaledKinds.size > 0 && !anyAliasMatch && selected.length < maxChunks) {
    ...
    for (const item of index.chunks) {
      if (selected.length >= maxChunks) break;
      if (selectedIds.has(item.chunk.id) || !signaledKinds.has(item.chunk.kind))
        continue;
