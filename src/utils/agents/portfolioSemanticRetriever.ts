...
const EMBEDDING_MODEL = process.env.PORTFOLIO_AGENT_EMBEDDING_MODEL ?? "text-embedding-3-small";
const EMBEDDING_DIMENSIONS = positiveInt(process.env.PORTFOLIO_AGENT_EMBEDDING_DIMENSIONS, 512);
const EMBEDDING_TIMEOUT_MS = positiveInt(process.env.PORTFOLIO_AGENT_EMBEDDING_TIMEOUT_MS, 6_000);
const DEFAULT_SEMANTIC_MIN_SIMILARITY = positiveFloat(process.env.PORTFOLIO_AGENT_SEMANTIC_MIN_SIMILARITY, 0.28);
...
export function canonicalPortfolioEmbeddingText(chunk: PortfolioChunk): string {
	return [`Title: ${chunk.title}`, `Source: ${chunk.source}`, `Aliases: ${chunk.aliases.join(", ")}`, chunk.text]
		.filter(Boolean)
		.join("\n");
}
...
function defaultEmbedder(apiKey: string): EmbedTexts {
	return async inputs => {
		...
		try {
			...
			if (ordered.length !== inputs.length) throw new Error("embedding_count_mismatch");
...
async function corpusEmbeddings(chunks: PortfolioChunk[], embed: EmbedTexts, cacheable: boolean): Promise<number[][]> {
	...
	if (cachedCorpusRef !== chunks || !cachedCorpusPromise) {
		...
		cachedCorpusPromise = embed(chunks.map(canonicalPortfolioEmbeddingText)).catch(error => {
...
async function queryEmbedding(query: string, embed: EmbedTexts, cacheable: boolean): Promise<number[]> {
	if (cacheable) {
...
export async function retrievePortfolioContextHybrid(
	...
	if (!embed) return lexicalFallback(lexical, "embedding_not_configured", false);

	const chunks = options.chunks ?? loadPortfolioChunks();
	if (chunks.length === 0 || !lexical.query.trim()) return lexicalFallback(lexical, "empty_corpus_or_query", false);
	...
	try {
		...
		if (corpus.length !== chunks.length) return lexicalFallback(lexical, "embedding_count_mismatch", true);

		const threshold = options.semanticMinSimilarity ?? DEFAULT_SEMANTIC_MIN_SIMILARITY;
		const semantic = chunks
			...
			.sort((a, b) => b.similarity - a.similarity || (b.chunk.priority ?? 0) - (a.chunk.priority ?? 0));
		if (semantic.length === 0) return lexicalFallback(lexical, "no_semantic_match", true);

		const lexicalById = new Map(lexical.chunks.map((chunk, index) => [chunk.id, { chunk, rank: index + 1 }]));
		const semanticById = new Map(semantic.map((item, index) => [item.chunk.id, { ...item, rank: index + 1 }]));
		...
		const ranked = [...candidateIds]
			...
			.filter((item): item is { chunk: RetrievedPortfolioChunk; fused: number } => item !== null)
			.sort((a, b) => b.fused - a.fused || a.chunk.id.localeCompare(b.chunk.id));
		...
		const reason = error instanceof Error ? error.message.slice(0, 80) : "semantic_retrieval_failed";
