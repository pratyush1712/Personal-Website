...
import { PORTFOLIO_AGENT_SOURCES } from "./portfolioSources";
...
const MAX_CHUNK_TEXT_CHARS = positiveInt(
  process.env.PORTFOLIO_AGENT_MAX_CHUNK_CHARS,
  2_400,
);
...
const METADATA_LABEL_RE =
  /^\s*(?:[-*]\s*)?(name|project|repository|repo|title|alias|aliases)\s*:\s*(.+)$/i;
const BOLD_LABEL_RE = /\*\*([^*\n]{2,40})\*\*\s*:/g;
const DECORATION_RE =
...
function isMeaningfulAlias(normalized: string): boolean {
  ...
  return tokens.some(
    (token) => !GENERIC_ALIAS_TOKENS.has(token) && !ALIAS_STOPWORDS.has(token),
  );
}
...
function splitSections(
  ...
  for (const line of raw.split(/\r?\n/)) {
    ...
    if (
      headingMatch &&
      isBoundary(headingMatch[1].length, headingMatch[2].trim())
    ) {
      if (
        current.lines.length > 0 ||
        current.title ||
        current.innerHeadings.length > 0
      )
    ...
    if (!inFence && headingMatch)
      current.innerHeadings.push(headingMatch[2].trim());
    current.lines.push(line);
  }
...
function pushAlias(into: string[], seen: Set<string>, candidate: string): void {
  ...
  if (!normalized || seen.has(normalized) || !isMeaningfulAlias(normalized))
    return;
  seen.add(normalized);
  into.push(cleaned);
}
...
function slugify(input: string): string {
  return (
    normalizeForMatch(input).replace(/\s+/g, "-").slice(0, 60) || "section"
  );
}
...
function buildChunk(
  ...
  const text = capText(
    cleanBody(section.lines.join("\n")),
    MAX_CHUNK_TEXT_CHARS,
  );
  const { aliases: strongAliases, seen } = collectStrongAliases(section);
  const aliases = [
    ...strongAliases,
    ...collectWeakAliases(section, seen),
  ].slice(0, MAX_RETRIEVAL_ALIASES);
  const entityAliases = strongAliases.slice(0, MAX_ENTITY_ALIASES);
  const hasBodyText = section.lines.some(
    (line) => line.trim() && !HEADING_RE.test(line),
  );
  if (section.isPreamble && !hasBodyText) return null;
  if (text.length < MIN_CHUNK_TEXT_CHARS && entityAliases.length === 0)
    return null;
...
function readDirMarkdown(relativeDir: string, fallback: string[]): string[] {
  try {
    const files = readdirSync(join(process.cwd(), relativeDir)).filter((file) =>
      file.endsWith(".md"),
    );
...
function loadAll(): ParsedChunk[] {
  ...
  for (const source of PORTFOLIO_AGENT_SOURCES) {
    const raw = readFileSafe(source.relativeDir, source.fileName);
    ...
    chunks.push(
      ...parseFile(
        raw,
        {
          ...
          isBoundary:
            source.boundary === "github-repositories"
              ? githubBoundary
              : headingsBoundary([2]),
        },
        usedIds,
...
function buildCache(): KnowledgeCache {
  ...
  const chunks = parsed.map(
    ({ entityAliases: _entityAliases, ...chunk }) => chunk,
  );
...
export function matchesDynamicPortfolioEntity(input: string): boolean {
  ...
  return getCache().entityMatchers.some(
    (alias) => alias.length > 0 && haystack.includes(` ${alias} `),
  );
}
