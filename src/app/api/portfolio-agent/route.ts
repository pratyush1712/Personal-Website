...
import {
  buildPolicyReminder,
  detectPortfolioRiskFlags,
} from "@/utils/agents/portfolioAgentPolicy";
import {
...
const MAX_CONTEXT_CHARS = positiveInt(
  process.env.PORTFOLIO_AGENT_MAX_CONTEXT_CHARS,
  12_000,
);
const OPENAI_MODEL = process.env.PORTFOLIO_AGENT_MODEL ?? "gpt-5.6-terra";
const OPENAI_FALLBACK_MODEL =
  process.env.PORTFOLIO_AGENT_FALLBACK_MODEL ?? "gpt-5.6-luna";
const OPENAI_MAX_OUTPUT_TOKENS = positiveInt(
  process.env.PORTFOLIO_AGENT_MAX_COMPLETION_TOKENS,
  900,
);
const OPENAI_REASONING_EFFORT = reasoningEffort(
  process.env.PORTFOLIO_AGENT_REASONING_EFFORT,
);
const PUBLIC_UNAVAILABLE =
  "Portfolio Agent is temporarily unavailable. Please try again shortly.";
const EMPTY_RESPONSE =
  "I could not produce a useful answer. Try a shorter or more specific question.";
...
function reasoningEffort(
  ...
  return value === "none" || value === "medium" || value === "high"
    ? value
    : "low";
}
...
async function startOpenAI(args: {
  ...
  if (
    ...
    isTransientOpenAIStatus(primaryStatus)
  ) {
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      try {
        ...
        if (!args.started.controller.signal.aborted && !wroteText)
          enqueue(PUBLIC_UNAVAILABLE);
        controller.close();
        finalStatus = args.started.controller.signal.aborted
          ? "cancelled"
          : "failed";
      } finally {
        logPortfolioAgentTelemetry({
...
export async function POST(req: NextRequest) {
  ...
  if (!latest)
    return json(
      { error: "The final message must be a non-empty user message." },
      400,
    );

  const rateLimit = await consumePortfolioRateLimit(req);
  const limitHeaders = portfolioRateLimitHeaders(rateLimit);
  if (!rateLimit.allowed) {
    return json(
      {
        error:
          "Message limit reached. Please wait briefly before trying again.",
      },
      429,
      limitHeaders,
    );
  ...
  try {
    ...
    if (body.stream === true)
      return createStreamingReply({ ...telemetry, headers: limitHeaders });
    ...
    console.error("[portfolio-agent] request failed", {
      ...
      providerStatus:
        error instanceof OpenAIStartError ? error.status : undefined,
    });
