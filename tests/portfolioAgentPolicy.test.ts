...
} from "../src/utils/agents/portfolioAgentPolicy";
...
test("risk detection flags injection without classifying the whole message as irrelevant", () => {
  ...
  assert.match(
    buildPolicyReminder(flags) ?? "",
    /answer that safe portion normally/i,
  );
});

test("ordinary portfolio and harmless general questions do not trigger risk flags", () => {
  assert.deepEqual(
    detectPortfolioRiskFlags("Would Pratyush be a good founding engineer?"),
    [],
  );
  assert.deepEqual(
    detectPortfolioRiskFlags("Explain black holes in two sentences."),
    [],
  );
});
