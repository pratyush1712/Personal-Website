...
import { assertPortfolioSourcesPresent } from "../src/utils/agents/portfolioKnowledge";

test("source manifest is explicit and excludes raw LinkedIn exports", () => {
	const paths = PORTFOLIO_AGENT_SOURCES.map(source => `${source.relativeDir}/${source.fileName}`);
...
test("raw LinkedIn export is not deployable and ignore rules prevent re-adding it", () => {
	assert.equal(existsSync(join(process.cwd(), "public/agent-context/LinkedInWhole.md")), false);
...
test("missing required evidence sources fail loudly", () => {
	assert.throws(
		() => assertPortfolioSourcesPresent(join(process.cwd(), "does-not-exist")),
		/Missing required portfolio agent source/
	);
});
