...
test("loads a CleverHug GitHub chunk with the expected shape", () => {
  ...
  const clever = chunks.find((chunk) =>
    chunk.aliases.some((alias) => /^cleverhug$/i.test(alias)),
  );
  ...
  assert.ok(
    /email scheduler/i.test(clever.text),
    "CleverHug text should describe the email scheduler",
  );
});
...
test("all 27 curated repositories become github-repo chunks (no fence swallowing)", () => {
  const githubChunks = loadPortfolioChunks().filter(
    (chunk) => chunk.kind === "github-repo",
  );
  assert.ok(
    githubChunks.length >= 27,
    `expected >= 27 github chunks, got ${githubChunks.length}`,
  );
});

test("loads curated advocacy evidence without loading a raw LinkedIn export", () => {
  const chunks = loadPortfolioChunks();
  const advocacy = chunks.find(
    (chunk) => chunk.kind === "advocacy" && /Disability:IN/i.test(chunk.text),
  );
  ...
  assert.ok(
    chunks.every(
      (chunk) =>
        !/LinkedInWhole|media\.licdn/i.test(`${chunk.source}\n${chunk.text}`),
    ),
  );
});
...
test("dynamic entities include CleverHug but exclude generic section words", () => {
  ...
  assert.ok(
    !entities.some((entity) =>
      /^(?:overview|features|description|usage|installation|architecture)$/i.test(
        entity,
      ),
    ),
...
test("matchesDynamicPortfolioEntity recognizes CleverHug and rejects unrelated text", () => {
  ...
  assert.equal(
    matchesDynamicPortfolioEntity("tell me about the project cleverhug"),
    true,
  );
  assert.equal(matchesDynamicPortfolioEntity("Explain black holes"), false);
  assert.equal(
    matchesDynamicPortfolioEntity("Write a LinkedIn post about crypto"),
    false,
  );
  assert.equal(
    matchesDynamicPortfolioEntity("Explain GitHub Actions generally"),
    false,
  );
});
