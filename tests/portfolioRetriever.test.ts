...
test("retrieves CleverHug for the GitHub-qualified phrasing", () => {
  const result = retrievePortfolioContext(
    "What is the project CleverHug on Pratyush's GitHub?",
  );
...
test("retrieves multiple sources for a disability-advocacy assessment", () => {
  const result = retrievePortfolioContext(
    "Is Pratyush an advocate for disability rights?",
  );

  assert.ok(result.chunks.some((chunk) => chunk.kind === "advocacy"));
  assert.ok(
    result.chunks.some((chunk) =>
      /Disability:IN|AccessComputing|Tapia/i.test(chunk.text),
    ),
  );
});
...
test("retrieves a Perfect Match chunk", () => {
  ...
  assert.ok(
    result.chunks.some(
      (chunk) =>
        /perfect-match/i.test(chunk.id) || /perfect match/i.test(chunk.text),
    ),
