...
test("semantic retrieval finds conceptually relevant evidence when lexical retrieval has no match", async () => {
	const embedTexts = async (inputs: string[]) =>
		inputs.map(input => {
			...
			return normalized.includes("equal access") || normalized.includes("disability and neurodiversity")
				? [1, 0]
				: [0, 1];
		});
	const result = await retrievePortfolioContextHybrid("Does his work promote equal access?", undefined, {
		chunks,
		embedTexts,
...
test("embedding failure preserves the deterministic lexical result", async () => {
	const result = await retrievePortfolioContextHybrid("What is his engineering focus?", undefined, {
		chunks,
		embedTexts: async () => {
